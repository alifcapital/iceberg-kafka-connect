...
public class RecordConverter {
  ...
  protected LocalTime convertTimeValue(Object value, String sourceSchemaName) {
    if (value instanceof Number) {
      ...
      // Handle ZonedTime ISO format like "10:15:30+01:00"
      String str = (String) value;
      if (str.contains("+") || str.contains("-") || str.endsWith("Z")) {
        // Parse as OffsetTime and extract LocalTime
        try {
          return java.time.OffsetTime.parse(str).toLocalTime();
        } catch (DateTimeParseException e) {
          // Fall back to LocalTime parsing
        }
      }
